...
const AdModal: React.FC<AdModalProps> = ({ campaign, onComplete, onSkip }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center text-white p-4 animate-fade-in-fast">
        <div className="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-lg shadow-2xl p-6 relative">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-yellow-400">Sponsored Message</h2>
                <div className="text-lg font-mono bg-black/30 px-3 py-1 rounded">
                    Reward in {timeLeft}s
                </div>
            </div>

            <div className="aspect-video bg-slate-800 rounded-md flex items-center justify-center mb-4 overflow-hidden">
                {renderMedia()}
            </div>
